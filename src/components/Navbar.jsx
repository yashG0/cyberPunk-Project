          ...
          <div className="hidden lg:flex items-center">
            <ul className="flex">
              {links.map(({ id, link }) => (
                <li
                  key={id}
                  className="p-4 uppercase duration-200 hover:text-thBlue cursor-pointer hover:translate-y-2"
                >
                  <Link to={link} smooth duration={500} onClick={()=>setIsMenuShown(false)}>
                    {link}
                  </Link>
                </li>
              ))}
